...
num_docs <- ndoc(final_copilot_corpus)  # Get the number of documents
new_docnames <- paste0("doc", seq_len(num_docs))  # Create names: doc1, doc2, ..., doc10

# Rename documents
docnames(final_copilot_corpus) <- new_docnames

# Verify the renaming
docnames(final_copilot_corpus)

# Step 1: Convert the corpus to a Document-Feature Matrix (DFM)
copilot_dfm <- dfm(final_copilot_corpus)

# Step 2: (Optional) Remove sparse terms (terms that appear in very few documents)
copilot_dfm_trimmed <- dfm_trim(copilot_dfm, min_termfreq = 5)

# Step 3: (Optional) Apply TF-IDF (Term Frequency-Inverse Document Frequency) to weight terms
copilot_dfm_tfidf <- dfm_tfidf(copilot_dfm_trimmed)
...
non_zero_variance_dfm <- copilot_dfm_tfidf[, apply(copilot_dfm_tfidf, 2, var) != 0]
